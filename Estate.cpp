...
Estate::Estate(int _positionx, int _positiony, int _price, int _level, int _processor)
	: Place(_positionx, _positiony)
{
	setInfoOfHouse(_price, _level, _processor);
}

void Estate::setInfoOfHouse(int _price, int _level, int _processor)
{
	house.price = _price;
	house.level = _level;
...
void Estate::PlaceFunction(Player& player)
{
	...
	// 再根据地产的状态执行操作
	// 如果这个地方的地产没有被购买
	if (this->getInfoOfHouse().processor == 0)
	{
		...
		if (player.getCash() >= this->getInfoOfHouse().price)
		{
			...
			if (isOK == IDOK)
			{
				...
				// 更改房产的相关信息
				this->setInfoOfHouse(this->getInfoOfHouse().price, 0, player.getNumber());
				setbkcolor(RGB(160, 238, 225));
				settextcolor(BLACK);
		...
		if (this->getInfoOfHouse().processor == player.getNumber())
		{
			...
			case 0:
			{
				...
				else
				{
					...
					if (isOK == IDOK)
					{
						...
						this->setInfoOfHouse(this->getInfoOfHouse().price, 1, player.getNumber());
						setbkcolor(RGB(160, 238, 225));
						settextcolor(BLACK);
						settextstyle(20, 0, "宋体");
			...
			case 1:
			{
				...
				else
				{
					...
					{
						player.update(this->house);
						this->setInfoOfHouse(this->getInfoOfHouse().price, 2, player.getNumber());
						setbkcolor(RGB(160, 238, 225));
						settextcolor(BLACK);
						settextstyle(20, 0, "宋体");
						outtextxy(300, 500, "你升级了地产！");
			...
			case 2:
			{
				...
				else
				{
					...
					{
						player.update(this->house);
						this->setInfoOfHouse(this->getInfoOfHouse().price, 3, player.getNumber());
						setbkcolor(RGB(160, 238, 225));
						settextcolor(BLACK);
						settextstyle(20, 0, "宋体");
						outtextxy(300, 500, "你升级了地产！");
